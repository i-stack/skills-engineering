...
expect_ok "validate_skill_evolution with SKIP bypasses step 8" \
  env SKIP_SNAPSHOT_CONSISTENCY=1 SKIP_BEHAVIOR_VALIDATION=1 bash scripts/validate_skill_evolution.sh

# ---- scripts/*.sh must all be executable ----
# 防止未来脚本因复制 / 重建丢失 +x 位导致 evolution 工作流静默损坏。
missing_exec_count=0
for s in scripts/*.sh; do
  if [ ! -x "$s" ]; then
    if [ "$missing_exec_count" -eq 0 ]; then
      echo "FAIL: scripts/*.sh missing +x:"
    fi
    echo "  - $s"
    missing_exec_count=$((missing_exec_count+1))
  fi
done
if [ "$missing_exec_count" -eq 0 ]; then
  pass=$((pass+1))
