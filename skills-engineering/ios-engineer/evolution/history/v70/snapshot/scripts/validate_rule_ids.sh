...
# Format: |  ID  |  status  |  ...
# Stops at the "## 退役记录" section to avoid double-counting retired entries.
index_entries = []
in_retired_section = false
File.foreach(index_file).with_index(1) do |line, lineno|
  in_retired_section = true if line.match(/^##\s+退役记录/)
  next if in_retired_section
...
missing_in_skill.each do |id|
  ...
  if status == "retired" || status == "deprecated"
    # Retired/deprecated IDs are expected to be absent from SKILL.md — skip.
    next
  end
  if id.start_with?("GR-")
    # GR-NNN rules are owned by global skills, not ios-engineer SKILL.md — exempt from bidirectional check.
    next
  end
