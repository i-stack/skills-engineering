...
if [ $# -lt 1 ]; then
  ...
  echo "in the surrounding response text. Covers IR-001 / GR-002 / GR-004 / IR-006 / GR-008 / GR-010 / IR-011"
  echo "(IR-002/IR-004/IR-008 deprecated — use GR-002/GR-004/GR-008; IR-010 deprecated — use GR-010). Other rule IDs are reported UNSUPPORTED."
  echo ""
...
SIGNALS = {
  ...
  "GR-002" => {
    check: ->(t) { !!(t =~ /^前置确认\s*$/m) },
    desc: "独立「前置确认」段标题（global engineering-discipline）"
  },
  "IR-002" => {
    check: ->(t) { !!(t =~ /^前置确认\s*$/m) },
    desc: "委托 GR-002（IR-002 已 deprecated，建议改用 GR-002）：独立「前置确认」段标题"
  },
  "GR-004" => {
    check: ->(t) {
    ...
    desc: "四段式（结论/为什么/修法/验证）或 findings-first 5 段全部存在（global engineering-discipline）"
  },
  "IR-004" => {
    check: ->(t) {
      four = %w[结论 为什么 修法 验证].all? { |k| t =~ /^#{Regexp.escape(k)}\s*$/m }
    ...
    desc: "委托 GR-004（IR-004 已 deprecated，建议改用 GR-004）：四段式或 findings-first 5 段全部存在"
  },
  "IR-006" => {
    check: ->(t) { !!(t =~ /^版本前提\s*$/m) },
    desc: "独立「版本前提」段标题"
  ...
  "GR-008" => {
    ...
    desc: "残留风险声明 + 已覆盖/未覆盖/残留风险 三字段全部存在（global engineering-discipline）"
  },
  "IR-008" => {
    check: ->(t) {
      t =~ /残留风险声明/ && t =~ /已覆盖/ && t =~ /未覆盖/ && t =~ /残留风险/
    },
    desc: "委托 GR-008（IR-008 已 deprecated，建议改用 GR-008）：残留风险声明 + 三字段全部存在"
  },
  "GR-010" => {
