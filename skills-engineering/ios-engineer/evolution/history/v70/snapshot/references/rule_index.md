...
| IR-002 | deprecated | 前置确认块已迁移至 GR-002（engineering-discipline global skill） | — |
| IR-003 | deprecated | 单根因锁定已迁移至 GR-003（engineering-discipline global skill） | — |
| IR-004 | deprecated | 四段式输出已迁移至 GR-004（engineering-discipline global skill）；PR review 例外保留在 ios-engineer OUT-002 | — |
| IR-005 | deprecated | 最小修复优先已迁移至 GR-005（engineering-discipline global skill） | — |
| IR-006 | active | 涉及并发 / 可用性 API / SwiftUI 行为 / 网络取消语义的输出，"结论"前必须有独立"版本前提"块（真值或显式假设），字段存在性可机械校验 | 同上 |
| IR-007 | deprecated | 不格式化代码已迁移至 GR-007（engineering-discipline global skill） | — |
| IR-008 | deprecated | 变更覆盖声明已迁移至 GR-008（engineering-discipline global skill） | — |
...
| IR-002 | deprecated | 前置确认规则提取为独立 global skill `engineering-discipline`，编号迁移至 GR-002；ios-engineer usage-audit 改用 GR-002。 | GR-002 | extract-engineering-discipline-global-skill |
| IR-003 | deprecated | 单根因锁定规则提取为独立 global skill `engineering-discipline`，编号迁移至 GR-003；ios-engineer usage-audit 改用 GR-003。 | GR-003 | extract-engineering-discipline-global-skill |
| IR-004 | deprecated | 四段式输出规则提取为独立 global skill `engineering-discipline`，编号迁移至 GR-004；PR review 例外保留在 ios-engineer OUT-002（路由至 review_checklists.md）；ios-engineer usage-audit 改用 GR-004。 | GR-004 | extract-engineering-discipline-global-skill |
| IR-005 | deprecated | 最小修复优先规则提取为独立 global skill `engineering-discipline`，编号迁移至 GR-005；ios-engineer usage-audit 改用 GR-005。 | GR-005 | extract-engineering-discipline-global-skill |
| IR-007 | deprecated | 不格式化代码规则提取为独立 global skill `engineering-discipline`，编号迁移至 GR-007；ios-engineer usage-audit 改用 GR-007。 | GR-007 | extract-engineering-discipline-global-skill |
| IR-008 | deprecated | 变更覆盖声明规则提取为独立 global skill `engineering-discipline`，编号迁移至 GR-008；ios-engineer usage-audit 改用 GR-008。 | GR-008 | extract-engineering-discipline-global-skill |
...
|------|-----------|---------|----------|
| 四段式输出（根因 → 为什么 → 修法 → 验证） | [engineering-discipline/SKILL.md](../../engineering-discipline/SKILL.md) GR-004 | [examples.md](examples.md) §1/§2/§4/§5/§6；[decision_records.md](decision_records.md) L5；[test_execution_and_repair.md](test_execution_and_repair.md) L82；[validation_scenarios.md](validation_scenarios.md) L26 / L88；[migration_strategy.md](migration_strategy.md)（剧本产物层） | 改 owner 必须同步所有引用；任一引用句式偏离 owner 即视为漂移 |
| findings-first 骨架（review 输出） | [review_checklists.md](review_checklists.md) §8 | [SKILL.md](../SKILL.md) OUT-002；[examples.md](examples.md) §3；[migration_strategy.md](migration_strategy.md) L114 | 改 owner 骨架字段必须同步 SKILL.md OUT-002 描述与 examples.md §3 引用句 |
...
| 前置确认块（IR-002 在信息不足时的字面化触发） | [engineering-discipline/SKILL.md](../../engineering-discipline/SKILL.md) GR-002 | [root_cause_enforcement.md](root_cause_enforcement.md) §2 取证策略"前置确认问题维度"小节；[scripts/lint_hit_rules.sh](../scripts/lint_hit_rules.sh) SIGNALS["GR-002"] | 改 owner 字面（如触发条件枚举）必须同步 root_cause_enforcement.md 维度示例；新增追问维度示例由对应 ROUTE 主读 ref 承担，不写进 owner，避免 SKILL.md 维度膨胀；段标题"前置确认"是机械校验 anchor，重命名需批量同步全部引用位置（含 lint_hit_rules.sh SIGNALS 表）|
