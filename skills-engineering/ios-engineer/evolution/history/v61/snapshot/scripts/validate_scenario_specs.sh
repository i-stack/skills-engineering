...
files.each do |file|
  ...
  if !expected_hits.is_a?(Array) || expected_hits.empty?
    ...
    expected_hits.each_with_index do |entry, idx|
      ...
      if entry.key?("rule_id")
        rid = entry["rule_id"]
        if !rid.is_a?(String) || rid !~ /\A[A-Z]+-\d{3}\z/
          violations << "#{file}: expected_hits[#{idx}].rule_id '#{rid.inspect}' must match ^[A-Z]+-\\d{3}$"
        end
      end
    end
  end
  ...
  if !failure_signals.is_a?(Array) || failure_signals.empty?
    ...
    failure_signals.each_with_index do |entry, idx|
      ...
      if entry.key?("rule_id")
        rid = entry["rule_id"]
        if !rid.is_a?(String) || rid !~ /\A[A-Z]+-\d{3}\z/
          violations << "#{file}: failure_signals[#{idx}].rule_id '#{rid.inspect}' must match ^[A-Z]+-\\d{3}$"
        end
      end
    end
  end
