...
- [IR-010] 回复必须具备可追溯的逻辑链：关键结论须能指向上游前提（事实、证据、或已陈述的推理步骤）；须区分「事实 / 推断 / 建议 / 推测」，不得把未验证推断写成定论；禁止无依据的因果跳跃、循环论证、同一回复内自相矛盾；非显然判断至少标出一步「因为…所以…」；证据不足时标明不确定，不得用流畅措辞伪装确定性。细则见 [logical_reasoning.md](references/logical_reasoning.md)。
