...
| IR-002 | active | 描述不清 / 上下文不足 / 歧义时先以独立"前置确认"块字面输出 ≥1 个具体问题，不允许仅在散文里说"需要更多信息" | 同上 |
...
| IR-010 | active | 回复须具备可追溯逻辑链；区分事实/推断/建议/推测；禁止因果跳跃、循环论证、自相矛盾；非显然判断至少一步「因为…所以…」 | 同上 |
...
| 逻辑性（可追溯论证 / 四层区分 / 推理可见） | [SKILL.md](../SKILL.md) IR-010 | [logical_reasoning.md](logical_reasoning.md); [scripts/lint_hit_rules.sh](../scripts/lint_hit_rules.sh) SIGNALS["IR-010"] | 改 owner 字面必须同步 logical_reasoning.md 六条标准与禁止表；机械锚点为「因为…所以…」或事实/推断/建议/推测层级词，不等价于完整逻辑证明，只用于发现明显漏写 |
