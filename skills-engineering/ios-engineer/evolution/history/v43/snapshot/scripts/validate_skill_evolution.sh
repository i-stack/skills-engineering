...
echo "[3/10] Validate referenced files exist"
missing=0
...
echo "[4/10] Validate layering guardrails"
...
echo "[5/10] Validate internal markdown links"
ruby <<'RUBY'
broken = 0
...
echo "[6/10] Validate scenario specs"
bash scripts/validate_scenario_specs.sh

echo "[7/10] Validate no orphan references"
ruby <<'RUBY'
referenced = {}
# SKILL.md 直接引用
...
echo "[8/10] Validate unique ownership + retired word regression"
ruby <<'RUBY'
...
echo "[9/10] Validate snapshot consistency with active version"
...
echo "[10/10] Run behavior validation scenarios"
