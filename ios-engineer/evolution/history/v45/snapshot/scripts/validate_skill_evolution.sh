...
echo "[3/11] Validate referenced files exist"
missing=0
...
echo "[4/11] Validate layering guardrails"
...
echo "[5/11] Validate internal markdown links"
ruby <<'RUBY'
broken = 0
...
echo "[6/11] Validate scenario specs"
bash scripts/validate_scenario_specs.sh

echo "[7/11] Validate rule IDs"
bash scripts/validate_rule_ids.sh

echo "[8/11] Validate no orphan references"
ruby <<'RUBY'
referenced = {}
# SKILL.md 直接引用
...
echo "[9/11] Validate unique ownership + retired word regression"
ruby <<'RUBY'
...
echo "[10/11] Validate snapshot consistency with active version"
...
echo "[11/11] Run behavior validation scenarios"
