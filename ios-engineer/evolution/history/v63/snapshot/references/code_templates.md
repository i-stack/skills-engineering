...
- 使用本文件模板落地到产线的代码交付（PR 描述 / 合入说明 / 交付报告），必须附带一个独立的"残留风险声明"块，固定三字段：已覆盖 / 未覆盖 / 残留风险（履行 IR-008）。三字段必须作为独立段落字面存在，不允许只写"已测试"或省略未覆盖项。与 [examples.md](examples.md) "残留风险声明"段字段对齐，保证四段式输出与产线代码交付两侧字段一致。
