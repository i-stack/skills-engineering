...
- ref 文件首行 `<!-- last-verified -->` 字段超过 12 个月（运行 [scripts/audit_ref_freshness.sh](../scripts/audit_ref_freshness.sh) 检测），且对应 ref 涉及 iOS / Swift / SwiftUI / Xcode 等会随系统迭代变化的内容。
...
每份 `references/*.md` 首行带 HTML 注释 `<!-- last-verified: YYYY-MM -->`，记录最近一次内容复核的年月。该字段是文件级元数据，与 IR / SYM / ROUTE / OUT 规则正交。

字段更新协议：
- 修改 ref 内容（除排版 / 链接修复外）后，必须把 last-verified 更新为当前年月；
- 内容未改但人工逐字复核了 iOS / Swift API 现状仍正确，也可主动更新（"复核但无内容变更"算一次有效复核）；
- 不允许在没有真正复核的情况下批量推时间戳。

审计周期：
- 建议每季度跑一次 [scripts/audit_ref_freshness.sh](../scripts/audit_ref_freshness.sh)；
- 默认阈值：`STALE_MONTHS=12`（标 STALE）/ `CRITICAL_MONTHS=18`（标 CRITICAL）；
