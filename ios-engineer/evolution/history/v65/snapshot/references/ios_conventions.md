...
- 本文件不预设 iOS / Swift 版本基线；并发写法、可用性 API、SwiftUI 行为类约束的具体取舍由实际工程的 `IPHONEOS_DEPLOYMENT_TARGET` 与 `SWIFT_VERSION` 决定。版本敏感建议详见 SKILL.md 核心铁律。
...
- 属性位置建议统一放在类结构末尾（初始化 / public API / private helpers 之后），避免不同访问级别的属性穿插分布。
