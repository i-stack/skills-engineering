...
- 若已经回放具体场景，使用 [scripts/record_validation_scenario.sh](../scripts/record_validation_scenario.sh) 把 `通过 / 部分通过 / 不通过`、命中点、偏差点和改进建议写入同一份验证记录；当所有场景均完成且结果满足条件时，提案可自动进入 `ready_to_promote`。
